...
from projects.brittle.utils.sparse_matrix_DFG import SparseMatrix, CGSolver
...
class DFGMPMSolver:
    ...
    def __init__(self, endFrame, fps, dt, dx, EList, nuList, gravity, cfl, ppc, vertices, particleCounts, particleMasses, particleVolumes, initialVelocity, outputPath, surfaceThreshold, useDFG = True, frictionCoefficient = 0.0, verbose = False, useAPIC = False, flipPicRatio = 0.0, symplectic = True, prescoredDamageList = []):
        ...
        # for backGrid in self.backGrid_sparse:
        #     block_component(backGrid)
    ...
    @ti.func
    def eigenDecomposition3D(self, M, location = 0):
    ...
    def backGridSort(self):
        ...
        for I in ti.grouped(self.pid):
            ...
            # print("hi:", self.backGrid[8,45,101,0]) #x = 8 doesnt even work
        ...
        for I in ti.grouped(self.pid):
            ...
            if self.useAnisoMPMDamageList[p] and self.useDFG:
                ...
                if ti.static(self.dim == 2):
                    ...
                    e, v1, v2, v3 = self.eigenDecomposition3D(kirchoff / J, 1) #use my eigendecomposition, comes out as four 3D vectors 
    ...
    def momentumP2GandComputeForces(self):
        ...
        for I in ti.grouped(self.pid):
            ...
            if ti.static(self.dim == 2):
                ...
                e, v1, v2, v3 = self.eigenDecomposition3D(kirchoff / J, 2) #use my eigendecomposition, comes out as four 3D vectors
    ...
    def computeContactForces(self):
        ...
        for I in ti.grouped(self.grid_m1):
            if self.separable[I] == 1: #only apply these forces to separable nodes with two fields
                ...
                #normals, make sure we normalize and treat 0 norms correctly
                n_1 = self.grid_n1[I]
                n_2 = self.grid_n2[I]
                if n_1.norm_sqr() == 0:
                    n_1 = ti.Vector.zero(float, self.dim)
                else:
                    n_1 = n_1.normalized()
                if n_2.norm_sqr() == 0:
                    n_2 = ti.Vector.zero(float, self.dim)
                else:
                    n_2 = n_2.normalized()
                ...
                if self.symplectic or not self.useImplicitContact: #only compute these contact forces for explicit sim
                    ...
                    #Compute tangent component for 2D vs 3D
                    fTanComp1 = ti.Vector.zero(float, self.dim)
                    fTanComp2 = ti.Vector.zero(float, self.dim)
                    ...
                    if ti.static(self.dim == 2):
                        ...
                        #Compute 3D orthonormal basis with n_cm1 = v1 NOTE: we have 6 cases here depending on zeros in n_cm1
                        s_cm1A = ti.Vector.zero(float, self.dim)
                        if v1x == 0 and v1y == 0 and v1z != 0: #(0 0 X)
                            s_cm1A = ti.Vector([1, 0, 0])
                        elif v1x == 0 and v1y != 0 and v1z == 0: #(0 X 0)
                            s_cm1A = ti.Vector([1, 0, 0])
                        elif v1x != 0 and v1y == 0 and v1z == 0: #(X, 0, 0)
                            s_cm1A = ti.Vector([0, 1, 0])
                        elif v1x == 0 and v1y == 0 and v1z == 0: #(0, 0, 0)
                            s_cm1A = ti.Vector.zero(float, self.dim)
                        elif v1x != 0 and v1y != 0 and v1z == 0: #(X, X, 0)
                            s_cm1A = ti.Vector([-1 * v1y, v1x, 0]).normalized() #similar to 2D case (do this to prevent div by 0 in case below)
                        else: #(X, X, X), (0, X, X), (X, 0, X)
                            #Let v2x = v1x, v2y = 2*v1y to prevent linear dependence on v1. Then set v3 based on solving v1 dot v2 = 0
                            v2x = 1 * v1x
                            v2y = 2 * v1y
                            v2z = -1 * (v1x**2 + (2 * (v1y**2))) / v1z
                            s_cm1A = ti.Vector([v2x, v2y, v2z]).normalized()

                        #third vector is always cross product of first two
                        s_cm1B = (n_cm1.cross(s_cm1A)).normalized()
                        ...
                        fTan1A = (m_1 / self.dt) * (v_cm - v_1).dot(s_cm1A)
                        fTan1B = (m_1 / self.dt) * (v_cm - v_1).dot(s_cm1B)
                        fTan2A = (m_2 / self.dt) * (v_cm - v_2).dot(s_cm2A)
                        fTan2B = (m_2 / self.dt) * (v_cm - v_2).dot(s_cm2B)

                        #Compute tangent component for each field
                        fTanComp1 = (fTan1A * s_cm1A) + (fTan1B * s_cm1B)
                        fTanComp2 = (fTan2A * s_cm2A) + (fTan2B * s_cm2B)

                    #Compute magnitude of tangent component for each field
    ...
    def ComputeResidual(self, project:ti.template()):
        ...
        for i in ti.ndrange(ndof):
            ...
            for d in ti.static(range(self.dim)):
                ...
                self.rhs[i*self.dim+d] *= self.zeroingFactor
        ...
        if self.useDFG:
            ...
            for i in range(sdof):
                ...
                for d in ti.static(range(self.dim)):
                    ...
                    self.rhs[ndof*self.dim + i*self.dim + d] *= self.zeroingFactor
    ...
    def BuildMatrix(self, project:ti.template(), project_pd:ti.template()):
        ...
        for i in ti.ndrange(ndof):
            ...
            self.entryVal[i*nNbr + midNbr] = ti.Matrix.identity(float, self.dim) * m * self.zeroingFactor
        for i in ti.ndrange(sdof):
            ...
            self.entryVal[(ndof + i)*nNbr + midNbr] = ti.Matrix.identity(float, self.dim) * m2 * self.zeroingFactor
            ...
            if ti.static(self.dim == 2):
                ...
                for i in range(3**self.dim):
                    ...
                    for j in range(3**self.dim):
                        ...
                        self.entryVal[ioffset] += dFdX * self.zeroingFactor
        ...
        if self.useDFG and self.useImplicitContact:
            for I in ti.grouped(self.grid_m1):
                if self.separable[I] == 1:
                    ...
                    self.entryVal[dof1*nNbr + midNbr] += self.factor * m00

                    #top right block (off diagonal)
                    self.entryCol[dof1*nNbr + (nNbr - sdof) + i2] = dof2
                    self.entryVal[dof1*nNbr + (nNbr - sdof) + i2] += self.factor * m01

                    #bottom left block (off diagonal)
                    self.entryCol[dof2*nNbr + (nNbr - sdof) + i2] = dof1
                    self.entryVal[dof2*nNbr + (nNbr - sdof) + i2] += self.factor * m10
    ...
    def SolveLinearSystem(self):
        ...
        self.avgCGIters[self.totalSubsteps] += iters
        ...
        for I in ti.grouped(self.grid_m1):
            if self.separable[I] == 1:
                ...
                if ci > 0: 
                    ...
                    dv1_guess = ti.Vector.zero(float, self.dim)
                    dv2_guess = ti.Vector.zero(float, self.dim)
                    #Add in the guess we just guessed
                    for d in ti.static(range(self.dim)):
                        dv1_guess[d] += self.DV[i*self.dim + d] #grab from first portion of DV
                        dv2_guess[d] += self.DV[ndof*self.dim + i2*self.dim + d] #grabbing from second field portion of DV

                    #Now compute vcm using updated v1 and v2
                    v_1_updated = v_1 + dv1_guess
                    v_2_updated = v_2 + dv2_guess
                    q_1 = v_1_updated * m_1
                    q_2 = v_2_updated * m_2
                    q_cm = q_1 + q_2 
                    m_cm = m_1 + m_2
                    ...
                    dv1 = (v_cm - v_1_updated).dot(n_cm1) * n_cm1
                    dv2 = (v_cm - v_2_updated).dot(n_cm2) * n_cm2
                    ...
                    #print("I:", I, "ciBefore:", ci, "ciNew:", ciNew)
    ...
    def checkConstraints(self):
        ...
        for i in range(self.separableNodes[None]):
            ...
            ci = self.gridCi[I]
            if ci <= 0:
                #print("constraint violated at grid index:", I)
    ...
    def implicitNewton(self):
        ...
        for iter in range(max_iter_newton):
            ...
            if iter == 0:
                ...
                if rnorm < 1e-8:
                    ...
                    self.newtonIters[self.totalSubsteps] = 0 #didn't even do CG!
                    break
            ...
            if ddvnorm < 1e-3:
                ...
                self.newtonIters[self.totalSubsteps] = iter + 1
                break
            ...
            if alpha == 1/2**15:
                print("[Line Search] ERROR: Check the direction! Ended at iter:", iter + 1)
                self.newtonIters[self.totalSubsteps] = iter + 1
                alpha = 0.0
                break
        ...
        if iter == max_iter_newton - 1:
            ...
            self.newtonIters[self.totalSubsteps] = max_iter_newton - 1
    ...
    #Routine to project all ci <= 0 to be ci > 0 in our initial guesses, TAG=Barrier
    @ti.kernel
    def projectCiDerivativeTest(self):
        #Now we must ensure that there are no Ci <= 0
        for I in ti.grouped(self.grid_m1):
            if self.separable[I] == 1:
                ndof = self.activeNodes[None]
                ...
                x1 = ti.Vector.zero(float, self.dim)
                x2 = ti.Vector.zero(float, self.dim)
                dx1 = ti.Vector.zero(float, self.dim)
                dx2 = ti.Vector.zero(float, self.dim)
                for d in ti.static(range(self.dim)):
                    x1[d] += self.dv[i*self.dim + d] #grab from first portion of dv (we projected initial guess into here)
                    x2[d] += self.dv[ndof*self.dim + i2*self.dim + d] #grabbing from second field portion of dv
                    dx1[d] += self.data_x[i*self.dim + d] #grab from first portion of data_x
                    dx2[d] += self.data_x[ndof*self.dim + i2*self.dim + d] #grabbing from second field portion of data_x

                #(ti.cast(ti.random(ti.f32), ti.f64) * 2.0 - 1.0) * self.delta[None] #resample randomly to find a better configuration

                maxIters = 5000

                #Now we iteratively loop projecting on x+dx and x-dx until we converge to a solution that works for both
                ciPlus = -1.0
                ciMinus = -1.0
                currIter = 0
                while ciPlus <= self.projectionThreshold or ciMinus <= self.projectionThreshold: #don't want ci too small to avoid numerical errors in barrier functions
                    ...
                    if currIter == maxIters:
                        ...
                        for d in ti.static(range(self.dim)):
                            dx1[d] = (ti.cast(ti.random(ti.f32), ti.f64) * 2.0 - 1.0) * self.delta[None]
                            dx2[d] = (ti.cast(ti.random(ti.f32), ti.f64) * 2.0 - 1.0) * self.delta[None]
                            currIter = 0
                            ciPlus = -1.0
                            ciMinus = -1.0

                    #Now we project x + dx and resave this into dx1 and dx2
                    #Compute Ci for x+dx
                    ...
                    if ci <= self.projectionThreshold:
                        ...
                        #Now with dv1 and dv2 that satisfy constraints for x + dx we must put this result in dx1 and dx2 properly for x - dx
                        dx1 = dv1 - x1
                        dx2 = dv2 - x2
                    ...
                    if ci <= self.projectionThreshold:
                        ...
                        #Now with dv1 and dv2 that satisfy constraints for x - dx we must save this result
                        dx1 = x1 - dv1
                        dx2 = x2 - dv2

                    #NOW finally let's test that this dx1 and dx2 satisfies BOTH constraints
                    ciPlus = (v_2 + (x2 + dx2) - v_1 - (x1 + dx1)).dot(n_cm1)
                    ciMinus = (v_2 + (x2 - dx2) - v_1 - (x1 - dx1)).dot(n_cm1)
                    # if ciPlus <= 0:
                    #     print("ciPlus <= 0:", ciPlus)
                    # if ciMinus <= 0:
                    #     print("ciMinus <= 0:", ciMinus)
                    # if ciPlus > 0 and ciMinus > 0:
                    #     print("Both constraints good!")

                    currIter += 1

                #Now put into data_x
                #print("Found a solution -- ciPlus:", ciPlus, "and ciMinus:", ciMinus)
                for d in ti.static(range(self.dim)):
                    self.data_x[i*self.dim + d] = dx1[d] 
                    self.data_x[ndof*self.dim + i2*self.dim + d] = dx2[d]
    ...
    def testGradient(self):
        ...
        if self.useImplicitContact: 
            self.computeCi()
            self.projectCi() #fix broken constraints and save to dv
            self.UpdateDV(0.0)
            self.computeCi() #compute updated ci based on DV (why we need to updateDV before this)
        self.UpdateState()
        E0 = self.TotalEnergy()
        print("E0 = ", E0)
        self.computeForces() 
        ...
        for _ in range(5):        
            ...
            if self.useImplicitContact:
                self.projectCiDerivativeTest() #compute constraints and then project data_x directly

            deltax = self.data_x.to_numpy()[0:tdof] #now grab deltax from data_x

            h = self.delta[None]
            self.RestoreStrain()
            self.UpdateDV(1.0) #put x + dx in DV
            ...
            #print("gradient E1: ", E1, "E2:", E2, "g1:", g1, "g2:", g2, "deltax", deltax)
                    ...
                    if self.useAnisoMPMDamage:
                        ...
                        if self.percentStretch > 0:
                            ...
                            if ti.static(self.dim == 2):
                                ...
                                e, v1, v2, v3 = self.eigenDecomposition3D(stretchKirchoff / stretchJ, 3)
                    ...
                    if self.useRankineDamage:
                        ...
                        if self.percentStretch > 0:
                            ...
                            if ti.static(self.dim == 2):
                                ...
                                e, v1, v2, v3 = self.eigenDecomposition3D(stretchKirchoff / stretchJ, 4)
                                stretchedSigma = e[0] #max in 3D
    ...
    #Second reset kernel because first one can only have 8 external args
    @ti.kernel
    def reset2(self,  prescoredDamageList: ti.ext_arr()):
        for i in range(self.numParticles):
            if self.prescoredDamage:
                self.Dp[i] = prescoredDamageList[i]
    ...
    def plotSolverDiagnostics(self, filepath):
        substeps = self.totalSubsteps
        cgIters = []
        newtonIters = []
        step = []
        for i in range(substeps):
            step.append(i)
            if self.newtonIters[i] != 0: 
    ...
    def simulate(self):
        ...
        with Logger(self.outputPath + "log.txt"):
            ...
            if not self.symplectic: self.plotSolverDiagnostics(self.outputPath + 'solverDiagnostics.png')  
            Timer_Print()