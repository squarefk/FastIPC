...
@ti.func 
def piola_FCR(F, R, J, mu, la):
  return 2 * mu * (F - R) @ F.transpose() + ti.Matrix.identity(float, 2) * la * J * (J - 1)
...
def substep():
  ...
  for p in x: 
    ...
    F[p] = (ti.Matrix.identity(float, 2) + dt * C[p]) @ F[p] #TODO: deformation gradient update
    ...
    #Compute Forces
    stress = piola_FCR(F[p], U@V.transpose(), J, mu, la)
  ...
  # Force and Velocity Updates + Collision
  for i, j in grid_m:
